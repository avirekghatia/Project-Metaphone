	  ...
	  if(isset($_GET['go'])){ 
		if(preg_match("/^[  a-zA-Z]+/", $_POST['name'])){ 
			  $name=$_POST['name']; 
				...
				$sql = "SELECT ConcertID, concertName, datetime FROM concert WHERE concertName = '$name'";
				//echo $sql;
				...
				<div class="dataGrid">
					...
					<div class="dataGridControl">
						<div class="dataGridControlButtons">
							<form  method="post" action="search_concerts.php?go"  id="searchform"> 
					...
					<table>
						...
						<tbody>
							...
							<?php } else {
									...
									$sql1 = "SELECT ConcertID, concertName, BandID, venueID, datetime FROM concert";
									//echo $sql1;
									$conn1 = new mysqli($servername, $username, $password, $dbname);
									...
									while($row1 = $result1->fetch_assoc()) {
										...
										$concertName = $row1["concertName"];
										$BandID = $row1["BandID"];
										$venueID = $row1["venueID"];
										$datetime = $row1["datetime"];
										
										
										/*$sql2 = "SELECT BandName FROM band WHERE BandID = $BandID";
										$sql3 = "SELECT Name, City FROM venue WHERE venueID = $venueID";
