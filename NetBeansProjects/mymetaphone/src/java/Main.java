...
class Input
{
    ...
    float appxpercent;
    //private ArrayList<String> trackedwords = new ArrayList<String>();
    ...
    /* array lists to store the fileids corresponding to each token of input query separately*/
    ...
    public void getInput(String lyric) // change function name later
    {
        ...
        // inputstr = s.nextLine();
        //inputstr = myMetaphoneInterface.setLyric();
        inputstr = lyric;
        ...
        //System.out.println("Modified string is :" + inputstr);
        parseInput(inputstr);
        try
        {
            Class.forName("com.mysql.jdbc.Driver");
    ...
    public int readDataBaseSearchQuery() throws SQLException
    {
        ...
        {
            Class.forName("com.mysql.jdbc.Driver");
            con = (Connection) DriverManager.getConnection("jdbc:mysql://localhost:3306/major", "root", "");
    ...
    private void searchWordList(String[] token) throws SQLException, ClassNotFoundException
    {
        ...
        try
        {
            ...
            for (i = 0; i < token.length; i++)
            {
                ...
                //"(wid integer, fid integer, in_title integer, in_verse integer, other integer)";
            ...
            ps1 = conn.prepareStatement("select fid from " + token[0]);
            ...
            for (i = 0; i < token.length; i++)
            {
                sq = "select tfid from fids" + i + " where tfid in(select fid from " + token[i] + ")";
                /* */
                System.out.println("sq :" + sq);
                ps = conn.prepareStatement(sq);
                ...
                //now putting fids of table token[0] in fids
                ps1 = conn.prepareStatement("select fid,in_title,in_verse,other from " + token[0]);
                res2 = ps1.executeQuery();
                ...
                for (i = 0; i < token.length; i++)
                {
                    ...
                    // sq = "select fid, in_title, in_verse, other from temp"+i+" where (fid, in_title, in_verse, other) in(select fid, in_title, in_verse, other from " + token[i]+ ")";
                    //sq = "select t1.fid, t1.in_title, t1.in_verse, t1.other from temp"+i+" as t1, "+token[i]+" as t2 where t1.fid = t2.fid AND (t1.in_title=t2.in_title OR t1.in_verse=t2.in_verse OR t1.other=t2.other)";
                    sq = "select t1.fid, t1.in_title, t1.in_verse, t1.other, t2.in_title, t2.in_verse, t2.other from temp" + i + " as t1, " + token[i] + " as t2 where t1.fid = t2.fid AND (t1.in_title=t2.in_title OR t1.in_verse=t2.in_verse OR t1.other=t2.other)";
                    System.out.println("sq :" + sq);
                    ps = conn.prepareStatement(sq);
                    res2 = ps.executeQuery();
                    while (res2.next())
                ...
                int rank = 1;
                while (res2.next()) {
                    sq = "select fid, fname, location from filelink where fid = " + res2.getInt(1);
                    ps = conn.prepareStatement(sq);
                    res = ps.executeQuery();
                    String filename;
                    if (res.next()) {
